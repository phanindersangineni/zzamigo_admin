...
import { DeleteAdvertisement, DeleteBannerImage, GetAllAdvertisements, GetAllBanner, GetAllLogos, GetAllTenants, deletLogoImages } from '../../../../services/CommonService';
...
// const AddLogo = lazy(() => import('./AddLogo'));
const Advertisement = () => {
  ...
  const loadData = async (pn) => {
    ...
    const respData = await GetAllAdvertisements(reqparams);
  ...
  const deleteData = async (item) => {
    const Response = await DeleteAdvertisement(item);
    if (Response.data.id) {
      ...
      <Col lg={12}>
        <Card>
          <Card.Header>
            ...
            <Card.Title>Advertisement <Button onClick={() => openModal(1, '')}
              ...
              <tbody>
                {data?.map(item => (
                  <tr>
                    <td>{item.position}</td>
                    <td>
                      <img style={{ "height": "100px" }} src={item.image} alt="Red dot" />
                    </td>
                    <td>{item.status}</td>
                    <td></td>
            ...
            <form >
              ...
              <div className="modal-body">
                ...
                <Suspense fallback={<div>Loading</div>}>
                  <AddAdvertisement  tenantprop={tenantdata} updateprop={updateitem}/>
                   </Suspense>
              </div>
