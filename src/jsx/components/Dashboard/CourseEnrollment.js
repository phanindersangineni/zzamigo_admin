...
import { GetAllAcademicsTypes, GetAllBranches, GetAllSemisters, GetAllUsersData, commonGet, 
 generateResume, getassessmentreport } from '../../../services/CommonService';
...
const CourseEnrollment = ({tenantid,role,tenantlist}) => {
    ...
    const searchData = async (pn, searchfields) => {
        ...
        setData([]);
        const respData = await getassessmentreport(reqparams);
        ...
        <Col lg={12}>
            <Accordion className="accordion accordion-danger-solid" defaultActiveKey="0">
                <Accordion.Item key={percent} eventKey={`${percent}`}>
                    <Accordion.Header className="accordion-header">
                        Search Course Enrolled Candidates
                    </Accordion.Header>
            ...
            <Card>
                <Card.Header>
                    ...
                    <Card.Title>All Course Enrolled Candidates
                      &nbsp; <u> <CSVLink
                            filename="all_Candidates_list"
                            data={csvdata}
                            headers={headers}>
                            Download Course Enrolled Candidates
                        </CSVLink> </u>
                ...
                <Accordion.Item key={percent} eventKey={`${percent}`}>
                    <Accordion.Header className="accordion-header">
                     Enrolled Candidate Data List
                    </Accordion.Header>
                    <Accordion.Collapse eventKey={`${percent}`} className="accordion__body">
                        <div className="accordion-body">

                {data.length > 0 &&
				<CourseEnrollmentTable propdata={data} />
               }
               </div>
                    </Accordion.Collapse>
                </Accordion.Item>
